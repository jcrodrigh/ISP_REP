...
import { DocumentGestion } from 'src/app/_models';
import { SortDirection } from '../../_utilitaries/document/document-table/sortable.directive';

interface SearchResult {
  documents: DocumentGestion[];
  total: number;
}
...
function sort(docs:DocumentGestion [], column: string, direction: string): DocumentGestion[] {
  if (direction === '') {
    return docs;
  } else {
...
function matches(doc: DocumentGestion, term: string, pipe: PipeTransform) {
...
export class DocumentManagementService {
  ...
  private _documents$ = new BehaviorSubject<DocumentGestion[]>([]);
  ...
  public documentsX:DocumentGestion[] = [];
